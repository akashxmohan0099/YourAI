...
export function BasicInfoStep({ tenantId, selectedTemplate, onTemplateSelect, onNext }: BasicInfoStepProps) {
  ...
  const inputClasses = 'w-full px-3 py-2 border border-zinc-300 rounded-lg text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-shadow'

  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      {/* Business type selector */}
      <div>
        <h2 className="text-lg font-semibold text-zinc-900">What type of business do you run?</h2>
        <p className="text-sm text-zinc-500 mt-1 mb-5">This helps us set up the right tools and templates for you.</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {BUSINESS_TYPE_TEMPLATES.map((template) => {
            ...
            return (
              ...
              >
                <IconComponent className={cn('w-5 h-5', isSelected ? 'text-indigo-600' : 'text-zinc-400')} />
      ...
      {selectedTemplate && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1.5">City / Suburb</label>
              <input
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-700 mb-1.5">Postcode</label>
                <input
